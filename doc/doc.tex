...
\subsection*{Long-lasting generation time differences are inconsistent with current
...
%\citet{gao2023limited} argue that these
%differences are unlikely to be driven by biological processes. Because such
%mutations occurred recently, these discrepancies would require strong
%mutation-type-specific selection or large recent shifts in the \emph{de novo}
%mutation spectrum.
...
bin, with the exact number depending on the variant-dating method. While
estimated mutation spectra from \GEVA, \tsdate and \relate differ at older
times, they closely agree for mutations inferred to be less than 100
generations old (Table~\ref{tab:recent-spectra}).
