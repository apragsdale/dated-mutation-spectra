...
\aprcomment{Possible titles:
    \begin{itemize}
        \item Inference of historical human generation times undone by
            multiple points of uncertainty
        \item On the fraught inference of historical human generation times
            [I still like this\dots]
        \item Others? \dots I'm bad at this
    \end{itemize}
}

Recent years have seen the rapid development of methods for reconstructing
\apredit{}{genetic} genealogical structures of large cohorts \krtcomment{is it clear that this means "unrelated"/
"randomly sampled" individuals and would not be confused with pedigrees?}
\aprcomment{I think it's ok-- I also added ``genetic'' to genealogical structures, which
should help}
...
\aprcomment{Easy to vary -- I added a table in the supplement with more scenarios.
There aren't any that help them out much.}
...
require $f=0.4$ or that the inferred genetion times in shared branch to be
...
S20 from \citet{wohns2022unified}). \apredit{}{Imperfect correlations should be expected,
...
Despite the low to moderate correlation, it is still possible that differences in
...
understood and properly accounted for, \apredit{}{we caution that}
...
existing methods show very different accuracies even under overly-idealized
...
These issues likely would have been identified by attempting to reproduce their
inference using available independent allele age estimates.
