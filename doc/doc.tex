...
\begin{figure}[t!]
    ...
    \caption{
        ...
        \krtcomment{Somewhere, it would be useful to note why tsdate isn't in here
        -- it is because it does actually date ALL mutations in the data (??),
        so that the inferred tree sequence is a lossless representation of the datums.}
    }
    \label{fig:data-comp}
\end{figure}
...
types. \krtcomment{Would it be helpful to give an example of how many SNPs these
...
\krtcomment{Here, we can say something like the mutation dating stuff is very new,
will probably be changing a good bit in coming years, and cite the paper from Rasmus'
group that the different methods have very different accuracies even under overly-idealized
conditions.}
